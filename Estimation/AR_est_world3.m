...
function [est_params,traj,contrib] = AR_est_world3(target,input_series,lags)
    import casadi.*
    ...
    % set up the auto-regressive model
    model = 0;
    for i = 1:lags
    ...
    traj = 0;
    for i = 1:lags
       traj = traj + est_params(i)*target(lags-i+1:T-i); % in sample prediction
       contrib(i) = mean(abs(est_params(i)*target(lags-i+1:T-i))); % measure of contribution to output from parameter i
    end
    for i = lags+1:lags+n_input_series
       traj = traj + est_params(i)*input_series(lags+1:T,i-lags); % in sample prediction
       contrib(i) = mean(abs(est_params(i)*input_series(lags+1:T,i-lags))); % measure of contribution to output from parameter i
    end
    contrib = contrib./sum(contrib); % normalize contributions
