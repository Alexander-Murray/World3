...
import casadi.* 
T0 = 1950;
dt=0.5; % persistent pollution equations misspecified if dt not 0.5
T = 125/dt;
skip_ML=false;
...
fraction_of_agricultural_inputs_alloc_to_land_development=SX.sym('fraction_of_agricultural_inputs_allocated_to_land_development',[T,1]);
...
industrial_material_emissions_factor=0.1;
    persistent_pollution_generation_factor_1=1.15;%1;%0;
    desired_completed_family_size_normal=3.3;%3.412;
...
initial_persistent_pollution=3e+07;%2.5e+07;
...
shift_resource_use_factor=(1985-T0)/dt;%shift_all;
...
shift_persistent_pollution_generation_factor=(1980-T0)/dt;%shift_all;
...
fertility_control_facilities_per_capita_fix = [0.0886879,0.0886879,0.0886879,0.0886879,0.08868839,...
...
perceived_life_expectancy_fix=[28.04366986,28.04366986,28.04366986,28.04366986,28.04379658,...
...
delayed_industrial_output_per_capita_fix=[41.5625,41.5625,41.5625,41.5625,...
...
delayed_industrial_output_per_capita=nan(T,1);
perceived_life_expectancy=nan(T,1);
fertility_control_facilities_per_capita=nan(T,1);
delayed_industrial_output_per_capita(1:max(1,1/(2*dt)):T)    = delayed_industrial_output_per_capita_fix((T0-1900)/2+1:max(1,2*dt):(T0-1900)/2+T*max(1,2*dt)/max(1,1/(2*dt)));
perceived_life_expectancy(1:max(1,1/(2*dt)):T)               = perceived_life_expectancy_fix((T0-1900)/2+1:max(1,2*dt):(T0-1900)/2+T*max(1,2*dt)/max(1,1/(2*dt)));
fertility_control_facilities_per_capita(1:max(1,1/(2*dt)):T) = fertility_control_facilities_per_capita_fix((T0-1900)/2+1:max(1,2*dt):(T0-1900)/2+T*max(1,2*dt)/max(1,1/(2*dt)));
delayed_industrial_output_per_capita = lin_interp(delayed_industrial_output_per_capita);
perceived_life_expectancy = lin_interp(perceived_life_expectancy);
fertility_control_facilities_per_capita = lin_interp(fertility_control_facilities_per_capita);
...
%     land_life_multiplier_from_land_yield(t)=land_life_multiplier_from_land_yield_1(t)+(land_life_multiplier_from_land_yield_2(t)-land_life_multiplier_from_land_yield_1(t))*arable_land_erosion_factor;
    ...
    fraction_of_agricultural_inputs_alloc_to_land_development(t)=fraction_of_agricultural_inputs_allocated_to_land_dev_table((marginal_productivity_of_land_development(t)/marginal_productivity_of_agricultural_inputs(t)));
    land_development_rate(t)=total_agricultural_investment(t)*fraction_of_agricultural_inputs_alloc_to_land_development(t)/development_cost_per_hectare(t);
    food_ratio(t)=(food_per_capita(t)/subsistence_food_per_capita);
    current_agricultural_inputs(t)=(total_agricultural_investment(t)*(1-fraction_of_agricultural_inputs_alloc_to_land_development(t)));
...
real_data = table2array(raw_data((T0-1900)*4+1:1*4*dt:end,10:end)); % start at column 10 since the first columns aren't real data. Skip by 2 since it is quarterly data
figure(1)
hold on
...
for v = 1:length(var_names)
    ...
    try
        eval(['sim_traj.(var_names{' num2str(v) '}) = full(evalf(' var_names{v} '))./normalization(' num2str(v) ');'])
    catch
