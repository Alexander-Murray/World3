...
% 	fraction_of_output_in_services(t)=service_output(t)/(PRICE_OF_FOOD*food(t)+service_output(t)+industrial_output(t));
    ...
    for v = 1:n_var %%%TO DO: this in a less hard-coded way
        ...
        elseif v==109 %family_income_expectation
            eval(['ineq2eq{t} = [ineq2eq{t};-0.5+' var_names{v} '{t}];']);
...
for tt = 1:est_iter_period-1:T-est_iter_period+1
    % initial state for each variable
...
%     if ~skip_ML
    ...
    init_var_vals = nan(n_var+4,1);
        ...
        init_var_vals(n_var+1) = infq_data(est_start+tt-1); %inflation_init_1
        init_var_vals(n_var+2) = infq_data(est_start+tt-2); %inflation_init_2
        init_var_vals(n_var+3) = infq_data(est_start+tt-3); %inflation_init_3
        init_var_vals(n_var+4) = infq_data(est_start+tt-4); %inflation_init_4
%     end
    ...
    for t = 1:minT
        ...
        eq_fun{t} = Function('eq',{params,init_vars,shifts,vertcat(shocks{1:t})},{[eq{t}]});
        ineq_fun{t} = Function('ineq',{params,init_vars,shifts,vertcat(shocks{1:t})},{[ineq2eq{t}]});
        no_shocks{t} = zeros(length(shocks{t}),1);
        
        if t<=est_iter_period
            eq_est{t} = eq_fun{t}(params,init_var_vals,shift_times-tt+1,vertcat(shocks{1:t}));
            ineq_est{t} = ineq_fun{t}(params,init_var_vals,shift_times-tt+1,zeros(length(vertcat(shocks{1:t})),1));
        end
    end
 ...
 % solve regression problem
    ...
    tic
    [est_params,traj,shock_sol] = nonlin_est_v4(non_nan_data(tt:tt+est_iter_period-1,:),eq_est,ineq_est,obs_var_inds,shocks,params,init_param_guess,param_lb,param_ub,opts);
...
for h = 1:H
    ...
    % run the simulation
    forecast_eq{h} = eq_fun{h}(param_means,sim_traj(:,T),shift_times-T-h+1,zeros(n_shocks*h,1));
end
...
for tt = 1:H:T
    for h = 1:H
        ...
        % run the simulation
        sim_eq{tt+h-1} = eq_fun{h}(param_means,sim_traj(:,tt),shift_times-tt-h+1,zeros(n_shocks*h,1));
    end
end
